//! Specifier keywords.

use crate::{
    lexis::token::{LeftParen, RightParen},
    list::SeparatedListDiagnostics,
    Parse, ParseError, ParseStream, Parser, PredictiveParse,
};

use super::Expr;

keyword! {
    KAbstract = "abstract",
    KAlwaysLoaded = "alwaysloaded",
    KAutoExpandCategories = "autoexpandcategories",
    KBitWise = "bitwise",
    KClassGroup = "classgroup",
    KClient = "client",
    KCoerce = "coerce",
    KCollapseCategories = "collapsecategories",
    KConfig = "config",
    KConst = "const",
    KCrossLevelActive = "crosslevelactive",
    KCrossLevelPassive = "crosslevelpassive",
    KDataBinding = "databinding",
    KDependsOn = "dependson",
    KDeprecated = "deprecated",
    KDontCollapseCategories = "dontcollapsecategories",
    KDontSortCategories = "dontsortcategories",
    KDuplicateTransient = "duplicatetransient",
    KEditConst = "editconst",
    KEditFixedSize = "editfixedsize",
    KEditHide = "edithide",
    KEditInline = "editinline",
    KEditInlineNew = "editinlinenew",
    KEditInlineUse = "editinlineuse",
    KEditorOnly = "editoronly",
    KEditTextBox = "edittextbox",
    KExec = "exec",
    KExpensive = "expensive",
    KExport = "export",
    KFinal = "final",
    KForceScriptOrder = "forcescriptorder",
    KGlobalConfig = "globalconfig",
    KHideCategories = "hidecategories",
    KHideDropdown = "hidedropdown",
    KImmutable = "immutable",
    KImmutableWhenCooked = "immutablewhencooked",
    KImplements = "implements",
    KInherits = "inherits",
    KInit = "init",
    KInput = "input",
    KInstanced = "instanced",
    KInterp = "interp",
    KIterationOptimized = "iterationoptimized",
    KIterator = "iterator",
    KLatent = "latent",
    KLocalized = "localized",
    KMulticast = "multicast",
    KNative = "native",
    KNativeReplication = "nativereplication",
    KNeverCook = "nevercook",
    KNoClear = "noclear",
    KNoExport = "noexport",
    KNoExportHeader = "noexportheader",
    KNoImport = "noimport",
    KNonTransactional = "nontransactional",
    KNoOwnerReplication = "noownerreplication",
    KNotForConsole = "notforconsole",
    KNotPlaceable = "notplaceable",
    KOptional = "optional",
    KOut = "out",
    KPerObjectConfig = "perobjectconfig",
    KPrivate = "private",
    KPrivateWrite = "privatewrite",
    KProtected = "protected",
    KProtectedWrite = "protectedwrite",
    KPlaceable = "placeable",
    KPublic = "public",
    KReliable = "reliable",
    KRepNotify = "repnotify",
    KSerialize = "serialize",
    KSerializeText = "serializetext",
    KServer = "server",
    KShowCategories = "showcategories",
    KSimulated = "simulated",
    KSingular = "singular",
    KSkip = "skip",
    KStatic = "static",
    KTransient = "transient",
    KVirtual = "virtual",
}

#[derive(Debug, Clone, PredictiveParse)]
pub struct SpecifierArgs {
    pub open: LeftParen,
    pub args: Vec<Expr>,
    pub close: RightParen,
}

impl Parse for SpecifierArgs {
    fn parse(parser: &mut Parser<'_, impl ParseStream>) -> Result<Self, ParseError> {
        let open: LeftParen = parser.parse()?;
        let (args, close) = parser.parse_comma_separated_list().map_err(|error| {
            parser.emit_separated_list_diagnostic(
                &open,
                error,
                SeparatedListDiagnostics {
                    missing_right: "missing `)` to close specifier argument list",
                    missing_right_label: "this `(` does not have a matching `)`",
                    missing_comma: "`,` or `)` expected after specifier argument",
                    missing_comma_open: "the specifier argument list starts here",
                    missing_comma_token:
                        "this was expected to continue or close the specifier argument list",
                    missing_comma_note: "note: specifier arguments must be separated by commas `,`",
                },
            )
        })?;
        Ok(Self { open, args, close })
    }
}
